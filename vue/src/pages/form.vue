...
export default {
	...
	data() {
		return {
			...
			rules: {
				...
				name: [{required: true, message: 'Điền vào tên người dùng', trigger: 'blur'},],
				bank: [{required: true, message: 'Vui lòng chọn tên ngân hàng', trigger: 'blur'},],
				bankNo: [{required: true, message: 'Điền số thẻ ngân hàng', trigger: 'blur'},],
			}
		};
	},
	components: {
	...
	methods: {
		...
		async ajaxData() {
			...
			let data = this.$http.post(`http://api.viettcl.com:8080/jeecg-boot/bank/user`, this.form)
			loading.close()
			return data
		},
		resetForm(formName) {
